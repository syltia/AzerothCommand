...
local function IsFrench() return GetLocale and GetLocale() == "frFR" end
local function Text(fr, en) if IsFrench() then return fr end return en end
...
local function EnsureThemeBeforeFrames()
    if AzerothAdmin and AzerothAdmin.db and AzerothAdmin.db.profile and AzerothAdmin.db.profile.azerothCommandTheme ~= 1 then
...
if AzerothAdminCommands then
    ...
    function AzerothAdminCommands.ReloadTable(tablename)
        ...
        if string.lower(tablename) == "all" then
            print(Text("Azeroth Command : '.reload all' est bloqué.", "Azeroth Command: '.reload all' is blocked."))
            return
        end
...
local function MiniButtons()
    return { ma_mm_mainbutton, ma_mm_charbutton, ma_mm_npcbutton, ma_mm_gobutton, ma_mm_telebutton, ma_mm_ticketbutton, ma_mm_miscbutton, ma_mm_serverbutton }
end
...
function AC.ApplyMiniMenuOrientation()
    ...
    if orientation == "HORIZONTAL" then
        ma_minibgframe:SetSize(198,28); ma_miniframe:SetSize(194,24)
        ma_mm_logoframe:ClearAllPoints(); ma_mm_logoframe:SetPoint("LEFT",ma_miniframe,"LEFT",3,0)
        local previous=ma_mm_logoframe
        for _,button in ipairs(MiniButtons()) do if button then button:ClearAllPoints(); button:SetPoint("LEFT",previous,"RIGHT",2,0); previous=button end end
    else
        ma_minibgframe:SetSize(28,244); ma_miniframe:SetSize(24,240)
        ma_mm_logoframe:ClearAllPoints(); ma_mm_logoframe:SetPoint("TOP",ma_miniframe,"TOP",0,-2)
        local previous=ma_mm_logoframe
        for _,button in ipairs(MiniButtons()) do if button then button:ClearAllPoints(); button:SetPoint("TOP",previous,"BOTTOM",0,-2); previous=button end end
    end
    ...
    if ma_ac_orientation_button then ma_ac_orientation_button:SetText(orientation=="HORIZONTAL" and Text("Barre horizontale","Horizontal bar") or Text("Barre verticale","Vertical bar")) end
end
...
local function CreateCustomMiscButtons()
    if not ma_midframe then return end
    if not ma_ac_orientation_button then
        local b=CreateFrame("Button","ma_ac_orientation_button",ma_midframe,"UIPanelButtonTemplate")
        b:SetSize(125,20); b:SetPoint("BOTTOMRIGHT",ma_midframe,"BOTTOMRIGHT",-140,8); b:SetScript("OnClick",AC.ToggleMiniMenuOrientation)
    end
    if not ma_ac_reset_theme_button then
        local b=CreateFrame("Button","ma_ac_reset_theme_button",ma_midframe,"UIPanelButtonTemplate")
        b:SetSize(100,20); b:SetPoint("BOTTOMRIGHT",ma_midframe,"BOTTOMRIGHT",-15,8); b:SetText(Text("Reset thème","Reset theme")); b:SetScript("OnClick",function() AC.ApplyDefaultTheme(true) end)
    end
end
...
local function SkinActionButtons()
    ...
    for _,group in pairs(FrameLib.group) do
        if type(group)=="table" then for _,frame in pairs(group) do
            if frame and frame.GetObjectType and frame:GetObjectType()=="Button" and frame.GetText and frame:GetText() then
                local name=frame:GetName() or ""
                if not string.find(name,"check") and not string.find(name,"dropdown") and frame.SetBackdrop then
                    frame:SetBackdrop({bgFile="Interface\\ChatFrame\\ChatFrameBackground",edgeFile="Interface\\Buttons\\WHITE8X8",tile=true,tileSize=8,edgeSize=1,insets={left=1,right=1,top=1,bottom=1}})
                    frame:SetBackdropColor(r,g,b,0.98); frame:SetBackdropBorderColor(math.min(1,r+0.18),math.min(1,g+0.18),math.min(1,b+0.18),1)
                end
            end
        end end
    end
...
local function CompactFrenchLabels()
    if not IsFrench() then return end
    local labels={
        ma_itembutton="Objet", ma_itemsetbutton="Set", ma_spellbutton="Sort",
        ma_skillbutton="Compétence", ma_questbutton="Quête", ma_objectbutton="GameObject",
        ma_creaturebutton="Créature", ma_telesearchbutton="Téléport", ma_sendmailbutton="Courrier",
        ma_inforefreshbutton="Actualiser"
    }
    for name,text in pairs(labels) do local f=_G[name]; if f and f.SetText then f:SetText(text) end end
end

local function ApplyBrandingAndSafety()
    -- Hide the original logo texture AND its texture child. The old logo must never sit behind the new title.
    if ma_logoframe then
        ma_logoframe:Hide()
        if ma_logoframe.texture then ma_logoframe.texture:Hide() end
        if ma_logoframe.GetRegions then for _,region in ipairs({ma_logoframe:GetRegions()}) do if region and region.Hide then region:Hide() end end end
    end
    if ma_topframe then
        if not ma_ac_brand then
            local brand=ma_topframe:CreateFontString("ma_ac_brand","OVERLAY","GameFontNormal")
            brand:SetPoint("TOPLEFT",ma_topframe,"TOPLEFT",18,-16); brand:SetFont(STANDARD_TEXT_FONT,30,"OUTLINE"); brand:SetText("Azeroth Command"); brand:SetTextColor(0.92,0.72,0.22,1)
        end
        ma_ac_brand:Show()
    end
    ...
    if ma_languagebutton then
        ...
        ma_languagebutton:SetScript("OnClick",function() AzerothAdmin:ChatMsg(".reload ale") end)
        ma_languagebutton:SetScript("OnEnter",function(self) GameTooltip:SetOwner(self,"ANCHOR_TOP"); GameTooltip:SetText("Reload ALE",1,.82,.25); GameTooltip:AddLine(Text("Recharge les scripts ALE/Eluna côté serveur.","Reload server-side ALE/Eluna scripts."),1,1,1,true); GameTooltip:Show() end)
    ...
    if ma_bgframe and ma_bgframe.SetBackdrop then
        ma_bgframe:SetBackdrop({bgFile="Interface\\DialogFrame\\UI-DialogBox-Background-Dark",edgeFile="Interface\\Tooltips\\UI-Tooltip-Border",tile=true,tileSize=16,edgeSize=14,insets={left=4,right=4,top=4,bottom=4}})
        ma_bgframe:SetBackdropColor(.015,.018,.025,.98); ma_bgframe:SetBackdropBorderColor(.42,.34,.12,1)
    end

    CreateCustomMiscButtons(); CompactFrenchLabels(); SkinActionButtons(); AC.ApplyMiniMenuOrientation()

    if ma_mm_logoframe then
        ma_mm_logoframe:RegisterForClicks("LeftButtonUp")
        ma_mm_logoframe:SetScript("OnClick",function() if IsShiftKeyDown() then ReloadUI() else AzerothAdmin:ToggleMiniMenu() end end)
        ma_mm_logoframe:SetScript("OnEnter",function(self) GameTooltip:SetOwner(self,"ANCHOR_LEFT"); GameTooltip:SetText("Azeroth Command",1,.82,.25); GameTooltip:AddLine(Text("Clic gauche : ouvrir/fermer","Left click: open/close"),1,1,1,true); GameTooltip:AddLine(Text("Orientation : bouton en bas de la fenêtre","Orientation: button at bottom of window"),.4,.85,1,true); GameTooltip:AddLine(Text("Ctrl + glisser : déplacer","Ctrl + drag: move"),.7,.7,.7,true); GameTooltip:AddLine(Text("Shift + clic : Reload UI","Shift + click: Reload UI"),.7,.7,.7,true); GameTooltip:Show() end)
...
if AzerothAdmin and AzerothAdmin.CreateFrames and not AzerothAdmin._AzerothCommandCreateFramesWrapped then
    ...
    function AzerothAdmin:CreateFrames(...)
        EnsureThemeBeforeFrames(); local result={UpstreamCreateFrames(self,...)}; ApplyBrandingAndSafety(); return unpack(result)
    end
...
login:SetScript("OnEvent",function() ApplyBrandingAndSafety() end)
